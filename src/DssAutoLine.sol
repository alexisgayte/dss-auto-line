...
contract DssAutoLine {
    ...
    */
    ...
    function _exec(bytes32 ilk, uint256 ilkLine, uint48  ilkTtl, uint48  ilkCtl, uint48  ilkLast, uint256 ilkGap) private returns (uint256, uint256) {
        ...
        require(lineNew <= line || block.number >= add(ilkLast, ilkTtl), "DssAutoLine/not-authorized-increase");
        require(lineNew >= line || block.number >= add(ilkLast, ilkCtl), "DssAutoLine/not-authorized-decrease");
